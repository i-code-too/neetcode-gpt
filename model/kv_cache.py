...
class KVCache:
    ...
    def update(self, new_k: torch.Tensor, new_v: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.cache_k is None or self.cache_v is None: # initializing on first call
            self.cache_k = new_k
            self.cache_v = new_v
        else: # appending on later calls
...
class CachedAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, kv_cache: Optional[KVCache] = None) -> Tuple[torch.Tensor, KVCache]:
        ...
        if kv_cache is None:
            kv_cache = KVCache()
        updated_K, updated_V = kv_cache.update(K, V)
        scores = (Q @ updated_K.transpose(-2, -1)) / (updated_K.shape[-1] ** 0.5)
        output = torch.softmax(scores, dim=-1) @ updated_V
