...
class MultiHeadedSelfAttention(nn.Module):
    ...
    def __init__(self, embedding_dim: int, attention_dim: int, num_heads: int):
        ...
        self.attention_heads = nn.ModuleList() # intitialize attention heads container
        for _ in range(num_heads):
            self.attention_heads.append(self.SingleHeadAttention(embedding_dim, attention_dim//num_heads))

        self.output_projection = nn.Linear(attention_dim, attention_dim, bias=False) # initialize output projection
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        return torch.round(self.output_projection(concatenated_outputs), decimals=4)
