...
class GPT(nn.Module):
    ...
    def forward(self, context: TensorType[int]) -> TensorType[float]:
        ...
        token_embeddings += self.position_embeddings(positions)
        transforms = self.transformer_blocks(token_embeddings)
        output = self.final_layer_norm(transforms)
        logits = self.vocabulary_projection(output)
