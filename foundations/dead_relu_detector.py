...
class Solution:
    ...
    def detect_dead_neurons(self, model: nn.Module, x: torch.Tensor) -> List[float]:
        ...
        with torch.no_grad():
            for layer in model.children():
                x = layer(x) # update new input to last output
                if isinstance(layer, nn.ReLU):
                    dead_fraction = (x == 0).all(dim = 0).float().mean().item() # fraction of dead neurons in each ReLU layer
    ...
    def suggest_fix(self, dead_fractions: List[float]) -> str:
        ...
        for dead_fraction in dead_fractions:
            if any(dead_fractions[i] > 0.5 for i in range(len(dead_fractions))):
